...
> ### ⚠️ 히어로 배경 색은 아래 값을 그대로 쓴다
> 사용자가 레퍼런스에서 직접 뽑아준 값이다. **임의로 단색화하지 않는다.**
> 단색(예: 주황 한 가지)으로 밀었다가 두 번 반려됐다.
>
> ```
> 배경(위→아래)  #A8A6C1  #C6C4D4  #CAC7D2  #EFCADD  #E1B5AC
> 파도            #DB898B  #AE6A61   (+ 검정이 섞임)
> ```
> 레퍼런스 배경은 **다색 저채도**다. 색 수를 줄이면 전혀 다른 인상이 된다.
...
직접 그려서 맞추려던 시도는 전부 실패했다.

| 시도 | 결과 |
|---|---|
| 라디얼 그라디언트 타원 | 가장자리가 매끈해 하나의 뭉개진 그림자로 읽힘 |
| 캔버스 사인파 곡선 | 화면 폭을 가로지르는 띠 → 풍경화의 지평선처럼 보임 |

**Paper Shaders의 `GrainGradient`가 `wave` 셰이프와 그레인을 함께 제공한다.**
레퍼런스와 같은 구성(물결 경계 + 거친 질감 + 다색 그라디언트)이 그대로 나온다.
...
- **색은 §2의 팔레트를 그대로 넣는다.** 단색으로 줄이면 전혀 다른 인상이 된다
- 파도 모양이 매 순간 달라지므로, 글이 놓이는 세로 구간(48% 중심)을 방사형 스크림으로
  통째로 눌러 **어느 위상에서도 흰 글씨가 읽히게** 한다
- 하단 34%를 `#0C0C0C`로 떨어뜨려 다음 섹션과 이음매를 없앤다
- `/lab`에서 다른 셰이더 후보와 비교할 수 있다 (noindex)
...
- 스태거 `i * 0.1s`
- **모집이 끝난 포지션은 번호·이름을 dim 처리하고 `모집 마감` 배지** 표시.
  카드 전체를 흐리게 하지 않는다 — 본문 대비가 §8 기준 아래로 떨어진다.
  지원 버튼 자리는 비우지 말고 마감 문구로 채운다
