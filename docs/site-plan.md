...
| `/admin` | **X** | 지원자 목록 + 안내 메일 발송 (§5.6). `ADMIN_TOKEN`으로 잠긴다 |
...
| `GET /api/admin/applications` | 지원자 전체 목록. `x-admin-token` 헤더 필요 |
| `POST /api/admin/notify` | 안내 메일 발송. `x-admin-token` 헤더 필요 |
...
| `상태` | Select | `접수됨` `서류 검토` `줌 미팅` `합류` `보류` |
| `한 줄 소개` | Rich text | |
| `지원 동기` | Rich text | |
| `관련 경험` | Rich text | |
...
| `미팅 일시` | Date | 운영진이 채운다. 안내 메일에 한국 시간으로 표기 |
| `줌 링크` | URL | 〃. 있으면 메일에 참여 버튼이 붙는다 |
| `안내 메시지` | Rich text | 〃. 메일 본문에 줄바꿈까지 그대로 실린다 |
| `안내 발송` | Rich text | 마지막 발송 기록. 서버가 채운다 |
| `제출일시` | Created time | |
...
| 포지션 | 필수 | 선택 |
|---|---|---|
| 기획/PM | 기능 우선순위 판단 기준 / 좋은 서비스란 | 관심 도메인 / 사용자 조사 방식 |
| Front-End | 시안 구현 난제 풀이 / API 지연·실패 처리 | 기억에 남는 화면 / 성능·접근성 경험 |
| Back-End | 오류 원인 추적 방식 / API 설계 경험 | 주 사용 스택 / FE와 명세·데이터 구조 이견 조율 |
| UI/UX Designer | Figma 수준과 협업 경험 / 사용성 피드백 대응 | 와이어프레임→UI 전개 / 완성도와 일정·기술 제약 조율 |
...
처음에는 `builditship.kro.kr`로 열었다가 짧은 주소로 옮겼다. 옮긴 뒤 한동안은 옛
주소를 301로 넘겨줬고, 그 주소로 메일을 받은 지원자가 모두 합류·불합격으로 정리된
뒤에 끊었다. Vercel에서 도메인을 떼고 DNS 레코드도 지웠다.

이름 자체는 내도메인.한국에 그대로 두었다. `kro.kr`은 선착순 무료라, 반납하면
제3자가 같은 이름을 주워 옛 홍보글의 유입을 가져갈 수 있다.
...
접수 확인 메일 말고, 심사가 진행될 때 지원자에게 보내는 메일이다.
**`/admin`에서 운영진이 직접 눌러 보낸다.**

```
노션에서 상태·미팅 일시·줌 링크·안내 메시지를 채운다
 └→ /admin 에서 운영자 키 입력 → 목록 조회
 └→ 지원자 행의 [안내 메일 보내기] → 내용 확인창 → 발송
 └→ 노션 `안내 발송`에 "2026-09-05 20:10 · 줌 미팅 안내 발송" 기록 + 본문에 이력 append
```

### 왜 자동 발송이 아닌가

노션 변경을 감지하려면 셋 중 하나가 필요한데, 어느 쪽도 공짜가 아니다.

| 수단 | 제약 |
|---|---|
| Vercel Cron | Hobby 플랜은 **하루 1회**가 최소 주기. 분 단위는 Pro |
| Notion 자동화 웹훅 | **유료 플랜 전용** |
| GitHub Actions 예약 | 무료지만 혼잡 시 5~30분 지연, 실행이 통째로 누락되기도 한다 |

무엇보다 **메일은 되돌릴 수 없다.** 노션에서 상태를 잘못 눌렀다 되돌리는 일은
흔하지만, 그 사이에 자동 발송이 나가면 수습할 방법이 없다. 지원자가 수십 명
규모라 사람이 누르는 비용도 크지 않다.

### 상태별 문구

제목과 첫 문장은 `src/lib/mail.ts`의 `STATUS_COPY`에 모여 있다.

| 상태 | 제목 |
|---|---|
| 서류 검토 | 서류 검토가 시작되었습니다 |
| 줌 미팅 | 줌 미팅 안내 |
| 합류 | 합류 안내 |
| 보류 · 접수됨 | 지원 진행 상황 안내 |

`미팅 일시`는 한국 시간으로 찍는다. 노션에서 **날짜만** 고르면 시각이 없으므로
"오전 9:00" 같은 없는 시간을 지어내지 않고 날짜까지만 적는다.
`안내 메시지`는 운영진이 쓴 문자열이 HTML에 들어가므로 반드시 이스케이프한다.

### 안내 메일에 수정 링크가 없는 이유

수정 토큰은 **원문을 저장하지 않는다**(§5). 접수 시점에 한 번 메일로 나가고
서버에는 해시만 남으므로, 나중에 같은 링크를 다시 만들 수 없다. 새로 발급하면
지원자가 이미 받은 링크가 죽는다. 그래서 안내 메일에는 링크를 넣지 않는다.

### 상태 문자열

노션 셀렉트 이름이 곧 상태값이라, 판정은 `src/lib/status.ts` 한 곳에서 한다.
띄어쓰기 차이(`줌미팅`)와 예전 이름(`커피챗`)을 흡수하므로 노션 표기가 조금
달라도 단계 표시와 메일 문구가 동시에 어긋나지 않는다.
...
| `ADMIN_TOKEN` | O | `/admin` 접근 키. **비우면 운영자 화면이 잠긴다** |
